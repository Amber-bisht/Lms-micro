...
export const connectDB = async (): Promise<void> => {
  try {
    ...
    logger.info('✅ Course Service - MongoDB connected successfully');
    logger.info(`[DB] Connected to database: ${mongoose.connection.db?.databaseName || 'unknown'}`);
  } catch (error) {
    logger.error(`❌ Course Service - MongoDB connection error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    process.exit(1);
  }
};
